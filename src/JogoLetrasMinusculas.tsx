...
import vetorletrasminusculas from "./letrasminusculas";

export default function JogoLetrasMinusculas() {
  ...
  function proximaLetra() {
    setLetra(vetorletrasminusculas[indice.current]);
    if (indice.current === vetorletrasminusculas.length - 1) {
      indice.current = 0;
    } else {
  ...
  useEffect(() => {
    shuffle(vetorletrasminusculas);
  }, [])

  return (
    <div className="bloco">
      <div className="container">
        <div className="titulo">Letras minúsculas</div>
        ...
        <div id="divdaletraminuscula" className="letra">
          {letra} <span id="letracursiva">{letra}</span>
        </div>
        <br />
